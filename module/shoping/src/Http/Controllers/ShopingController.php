...
class ShopingController extends BaseController
{
    ...
    public function getIndex(){
        $listPackage = $this->pa->orderBy('is_order','asc')
            ->findWhere(['status'=>'active','type'=>'product'])->all();
        $listService = $this->pa->orderBy('is_order','asc')
            ->findWhere(['status'=>1,'type'=>'service'])->all();
        return view('nqadmin-shoping::index',[
    ...
    public function getDetail($id){
        ...
        $diferencePackage = $this->pa->orderBy('is_order','asc')
            ->scopeQuery(function ($q) use($id,$data){
            return $q->where('id','!=',$id)->where('type',$data->type);
        })->all();

        $product = Product::with(['getPackage'=>function($e) use($id,$data){
