...
$(document).on("ready", function(){
    ...
    if($(window).width() >= 1024 && body_1.hasClass("no-menu-show") != true){
    ...
    if ($('.trip-icon-list').length > 0) {
	    $('.trip-icon').click(function (e) {
		    e.preventDefault();
		    var _this = $(this);
		    $('.trip-icon').removeClass('selected-icon');
		    _this.addClass('selected-icon');
		    var value = _this.children('i').attr('class');
		    var input = $('#icon');
		    input.val(value);
	    })
    }
