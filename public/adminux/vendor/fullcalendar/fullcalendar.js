...
Grid.mixin({
	...
	buildBusinessHourEvents: function(wholeDay, businessHours) {
		...
		if (businessHours == null) {
			// fallback
			// access from calendawr. don't access from views. doesn't update with dynamic options.
	...
	// Attaches event-element-related handlers for *all* rendered event segments of the views.
...
DayGrid.mixin({
	...
	showSegPopover: function(row, col, moreLink, segs) {
		...
		options = {
			...
			parentEl: this.view.el, // attach to root of views. guarantees outside of scrollbars.
...
var View = FC.View = Class.extend(EmitterMixin, ListenerMixin, {
	...
	// Sets the container element that the views should render inside of, does global DOM-related initializations,
...
var Calendar = FC.Calendar = Class.extend({
	...
	buildViewSpecOptions: function(spec) {
		spec.options = mergeOptions([ // lowest to highest priority
			...
			this.dirDefaults,
			this.localeDefaults, // locale and dir take precedence over views's defaults!
	...
	buildViewSpecButtonText: function(spec, requestedViewType) {
		...
		function queryButtonText(options) {
			...
			return buttonText[requestedViewType] ||
				// views can decide to look up a certain key
...
function Calendar_constructor(element, overrides) {
		...
		if (currentView) {
			...
			if (
				...
				if (elementVisible()) {
					...
					// NOTE: views updates title text proactively
	...
	function rerenderEvents() { // API method. destroys old events if previously rendered.
		if (elementVisible()) {
			t.reportEventChange(); // will re-trasmit events to the views, causing a rerender
		}
	}
	...
	function setOptions(newOptionHash) {
		...
		// catch-all. rerender the header and footer and rebuild/rerender the current views
		renderHeader();
		renderFooter();
...
// Return events objects for business hours within the current views.
// Abuse of our event system :(
Calendar.prototype.getCurrentBusinessHourEvents = function(wholeDay) {
	return this.computeBusinessHourEvents(wholeDay, this.options.businessHours);
};

// Given a raw input value from options, return events objects for business hours within the current views.
...
Its "el" is the inner-content of the above views's scroller.
*/
var ListViewGrid = Grid.extend({
	...
	// render the event segments in the views
