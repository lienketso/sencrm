((window, factory) ->
  ...
  class Tour
    ...
    _scrollIntoView: (step, callback) ->
      ...
      @_debug "Scroll into views. ScrollTop: #{scrollTop}. Element offset: #{offsetTop}. Window height: #{windowHeight}."
      counter = 0
    ...
    _showOverlayElement: (step, force) ->
      ...
      if !@backdrop.overlayElementShown
        ...
        @backdrop.$main-background = $ '<div>', class: 'tour-step-background'
        @backdrop.$main-background.appendTo(step.backdropContainer)
    ...
    _hideOverlayElement: ->
      ...
      @backdrop.$main-background.remove()
      @backdrop.$element = null
      @backdrop.$main-background = null
